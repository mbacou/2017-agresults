...
FEWS spatial data layers are distributed through multiple channels, primarily USGS and NOAA data portals. There is a series of interactive viewers and tools to explore and download slices of FEWS' climatic indicators, e.g.:
...
In this study all spatial estimates are derived from raw data at the following locations:
...
[aWhere](http://awhere.com/) is a US for-profit with simplified access to current (up to 30 months) global weather conditions at registered field locations. Recently aWhere launched an [R client](http://blog.awhere.com/using-awheres-open-source-weather-data-tools) to access their APIs. The service is *not* free (and acccording to their literature it is not meant to collect data at hundred or thousands of point locations). Similar information is available *for free* from AfSIS (see below).
...
# All layers are projected!
# +proj=laea +lat_0=5 +lon_0=20 +x_0=0 +y_0=0 +units=m +ellps=WGS84 +datum=WGS84
# A REST API is available for point queries
# https://rest.soilgrids.org/query?lon=5.39&lat=51.57&attributes=ORCDRC,CEC&depths=sl1
# FTP location at ftp://ftp.soilgrids.org/data/recent/
# Downloaded manually (not too many layers)

afsis.prj <- "+proj=laea +lat_0=5 +lon_0=20 +x_0=0 +y_0=0 +units=m +ellps=WGS84 +datum=WGS84"
tmp <- raster("~/Projects/hc-data/AfSIS/TAXNWRB_250m_ll.tif")
minValue(tmp)
# [1] 0
maxValue(tmp)
# [1] 255 => stretched again? Need documentation
spplot(tmp)
tmp <- crop(tmp, pts)

proj4string(tmp) <- afsis.prj
tmp <- projectRaster(tmp, proj4string(pts))

m <- tm_shape(tmp) + tm_raster(names(tmp), pal=pal.ndvi(255),
  title="Dominant Soil Texture (AfSIS 250m)") + p
...
Data on soil characteristics is obtained from ISRIC Africa Soil Information Service ([AfSIS](http://www.isric.org/projects/africa-soil-information-service-afsis)). AfSIS generates maps of soil properties at 250m and 1km resolution using legacy soil profile data (from about 12,000 locations in Africa) combined with remote sensing information (climate, land cover, population density, biodiversity, and other covariates). All indicators are available for download at http://geonode.isric.org/ with a web viewer at https://www.soilgrids.org/. A [REST SoilGrids API](https://rest.soilgrids.org/query.html) allows retrieving data at point locations. We are retrieving the following layers:

- `ORCDRC` soil organic carbon content  
- `TAXGWRBMajor` predicted most probable soil class
...
```{r afsis-orcdrc, fig.cap="Soil organic Carbon Content (depth 5-10cm). Source: AfSIS 250m."}
