...
```{r res-fews, fig.cap="Distribution of Last Period Bioclimatic Indicators at Household Locations across Nigeria States and Kenya Regions."}

pplot(rfe~adm1_name*adm0_name, pts.dekadal[date=="2017-06-01"], 
  ylab="Dekadal Rainfall Estimate (mm, FEWSNET/RFE)", xlab=NA)
pplot(chirps~adm1_name*adm0_name, pts.monthly[date=="2017-05-15"], 
  ylab="Monthly Rainfall (mm, CHIRPS)", xlab=NA)
pplot(ndvi~adm1_name*adm0_name, pts.dekadal[date=="2017-06-01"], 
  ylab="NDVI (-1,1 index, eMODIS C6)", xlab=NA, ylim=c(-0.1,1))
pplot(ndvi_anom~adm1_name*adm0_name, pts.dekadal[date=="2017-06-01"], 
  ylab="NDVI Anomaly (percent of median, eMODIS C6)", xlab=NA, ylim=c(50,150))
pplot(eta~adm1_name*adm0_name, pts.dekadal[date=="2017-06-01"], 
  ylab="Actual Evapotranspiration (mm, MODIS/SSEBop)", xlab=NA)
pplot(pet~adm1_name*adm0_name, pts.dekadal[date=="2017-06-01"], 
  ylab="Potential Evapotranspiration (mm, NOAA/GDAS)", xlab=NA)  

```

We note from the above charts that NDVI and NDVI Anomaly values should be corrected on the left tail (might want to set all NDVI values below -0.3 to NA). These only affect the last 3 dekades (mostly).
...
kable(pts.dekadal[, .(
  ...
  caption="Dekades with NDVI values below -0.3")
...
pplot(I(bio4/100)~adm1_name+adm0_name, pts.wc, 
  ylab="Temperature Seasonality (celsius, std. dev.)", xlab=NA)
pplot(bio15~adm1_name+adm0_name, pts.wc, 
...
pplot(orcdrc_m_sl1_250m_ll~adm1_name+adm0_name, pts.soil, 
  ylab="Soil Organic Carbon (0 cm, permilles)", xlab=NA)
pplot(orcdrc_m_sl4_250m_ll~adm1_name+adm0_name, pts.soil, 
...
pplot(tt10_20k~adm1_name+adm0_name, pts.tt, 
  ylab="Travel Time to 20K Market (hrs)", xlab=NA, beside=F)
pplot(tt10_100k~adm1_name+adm0_name, pts.tt, 
