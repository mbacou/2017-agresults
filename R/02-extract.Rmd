...
for (i in c("NGA", "KEN")) {
  ...
  pplot(eta~adm1_name, data=pts.dekadal[ISO3==i & date=="2017-06-01"], 
    ylab="Actual Evapotranspiration (mm, MODIS/SSEBop)", xlab=NA)
  pplot(pet~adm1_name, data=pts.dekadal[ISO3==i & date=="2017-06-01"], 
...
kable(pts.dekadal[, .(
  ...
  `NDVI Anomaly < 30%` = sum(ndvi_anom < 30, na.rm=T)), by=.(ISO3, date)][
    `NDVI < -.3` > 0 | `NDVI Anomaly < 30%` > 0],
