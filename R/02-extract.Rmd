...
kable(lbl[, .(
  Variable=varCode, 
  Description=paste0(varLabel, "\n", url),
  Period=period, Unit=unit, File=file, Start=start, End=end, Resolution=res)],
  align="llcclccc", caption="Codebook of Indicators")
...
Below are distribution graphs of a sample of indicators[^violin]. All estimates below are at surveyed household locations (after imputing missing/invalid locations as described in [Section 1](./index.html)).

[^violin]: The *violin* (also called "pirate") plots in this document show **median** line in **red**, **mean** line in **green**, and the **blue** region is the inferred **95% confidence interval** of the mean.
...
# 2) NDVI stretch and nil cloud cover of last dekad
...
# Roll by dekad and take the mean (from 2015-01-01 to 2017-06-30)
...
All results are available in **STATA format** on [Huddle](https://abtassociates.huddle.com/workspace/734056/files/#/folder/1170295/list). This section only shows summaries for a sample of indicators.

STATA files are organized by country, by source, and by periodicity:

- `2017-agresults-{ISO3}-biovars.dta` -- 1970-2000 baseline bioclimatic indicators (source: WorldClim), altitude and slope (source: SRTM)
- `2017-agresults-{ISO3}-traveltime.dta` -- travel time to nearest market (5 market sizes) (source: IFPRI/HarvestChoice)
- `2017-agresults-{ISO3}-soil.dta` -- soil organic carbon content and most probable soil class (source: SoilGrids)
- `2017-agresults-{ISO3}-monthly.dta` -- 2015-2017 monthly bioclimatic indicators (source: FEWSNET/USGS/MODIS)
- `2017-agresults-{ISO3}-dekadal.dta` -- 2015-2017 dekadal bioclimatic indicators (source: FEWSNET/USGS/MODIS)

All extracted indicators are listed below. This table is also on [Huddle](https://abtassociates.huddle.com/workspace/734056/files/#/folder/1170295/list).


## Bioclimatic Indicators

For documentation's purposes we graph the distributions of a sample of indicators across household locations.

<u>Note</u>: NDVI values below -0.3 should be null.

```{r res-fews, fig.cap="Distribution of Last Period Bioclimatic Indicators at Household Locations across States/Regions."}

par(mfrow=c(1,2))
for (i in c("NGA", "KEN")) {
  pplot(rfe~adm1_name, data=pts.dekadal[ISO3==i & date=="2017-06-01"], 
    ylab="Dekadal Rainfall Estimate (mm, FEWSNET/RFE)", xlab=NA)
  pplot(chirps~adm1_name, data=pts.monthly[ISO3==i & date=="2017-05-15"], 
    ylab="Monthly Rainfall (mm, CHIRPS)", xlab=NA)
  pplot(ndvi~adm1_name, data=pts.dekadal[ISO3==i & date=="2017-06-01"], 
    ylab="NDVI (-1,1 index, eMODIS C6)", xlab=NA, ylim=c(-0.3,1))
  pplot(ndvi_anom~adm1_name, data=pts.dekadal[ISO3==i & date=="2017-06-01"], 
    ylab="NDVI Anomaly (percent of 2007-2016 median, eMODIS C6)", xlab=NA)
}
...
par(mfrow=c(1,2))
for (i in c("NGA", "KEN")) {
  pplot(I(bio4/100)~adm1_name, data=pts.wc[ISO3==i], 
