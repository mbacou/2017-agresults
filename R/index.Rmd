...
There are **`r nrow(nga.odd)` households** with GPS coordinates that do not fall within their recorded LGA. In the majority of instances the distance is negligeable (under 10km) and this might not affect any of the spatial estimates. In a few instances the errors are troublesome (over 40km). For documentatio purposes households with problematic locations are highlighted on the map below.
...
**`r format(nrow(ken.hh), big.mark=",")` households** are sampled in Kenya. There are **`r ken.hh[is.na(X), .N]`** households missing coordinates and **` nrow(ken.odd)` possibly invalid** locations.
