---
title: "Geospatial Characteristics of AgResults Beneficiaries"
...
opts_chunk$set(comment=NA, warning=F, message=F, echo=F, base.url="../docs",
  dev="png", fig.path="fig/", fig.width=7, fig.height=3.8, dpi=300, cache=T, 
  dev.args=list(
...
m.nga <- tm_shape(nga.l2) +
  ...
  tm_dots(col="calc_lga", shape=25, alpha=.5, border.col="white",
    legend.show=T, size=.6, palette=viridis(22),
    title="Nigeria\nHousehold Locations\n(uncertain)") +
...
# Kenya Locations
#####################################################################################
# 2017.07.01 update: received admin details from Betsy but file is no good
...
m.ken <- tm_shape(ken.l2) +
  ...
  tm_dots(col="adm2_name", shape=25, alpha=.5, border.col="white",
    legend.show=T, size=.3, palette=viridis(24), max.categories=24,
    title="Kenya\nHousehold Locations") +
...
The tables and maps below provide details of biophysical, socio-economics, and market-based characteristics of farm households interviewed as part of AgResults' baseline and endlines surveys in Nigeria, Kenya, and Uganda.

In particular we highlight sampled households with missing and/or invalid spatial locations. In the case of missing coordinates a location is imputed using the centroid of this household's administrative unit (LGA for Nigeria, county for Kenya, and district for Uganda). Note that another approach would be to derive average statistics over the entire administrative unit, however this approach is more resource-intensive.
...
There are **`r nrow(nga.odd)` households** with GPS coordinates that do not fall within their recorded LGA. In the majority of instances the distance is negligeable (under 10km) and this might not affect any of the spatial estimates. In a few instances the errors are troublesome (over 40km). Households with problematic locations are highlighted on the map below.


```{r nga-odd, fig.cap="Households Located outside of their recorded LGA, Nigeria", fig.height=6, fig.width=6}
...
**`r prettyNum(nrow(ken.hh))` households** are sampled in Kenya. There are **`r ken.hh[is.na(X), .N]`** households missing coordinates and **` nrow(ken.odd)` possibly invalid** locations.
...
**` prettyNum(nrow(uga.hh))` households** are sampled in Uganda. There are **` uga.hh[is.na(X), .N]`** households missing coordinates and **` nrow(uga.odd)` possibly invalid** locations.
